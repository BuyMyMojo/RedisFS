...
fn download_file(redis_address: String, out_path: String, file_key: String) {
    ...
    // Connect to Redis
    let client = redis::Client::open(redis_address).expect("Failed to create Redis client");
    let mut con = client.get_connection().expect("Failed to connect to Redis");

    // Store the file in the DB
    let contents: Vec<u8> = con
        .get(format!("RedisFS:{}", &file_key))
...
fn upload_file(redis_address: String, file_path: String, file_key: String) {
    ...
    // Store the file in the DB
    let _: () = con
        .set(format!("RedisFS:{}", &file_key), buffer)
...
fn file_memory_usage(redis_address: String, file_key: String) {
    ...
    // Store the file in the DB
    let usage: f64 = redis::cmd("MEMORY")
        .arg("USAGE")
        .arg(format!("RedisFS:{}", file_key))
        .query(&mut con)
